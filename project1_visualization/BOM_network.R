...
library(networkD3)#for network plots
...
temp_df=select(movie_nodes,src=name,id,src_genre=genre)#temporary frame to make the id join easier
movie_links=inner_join(movie_links, temp_df, by = "src")#do the id join
movie_links=select(movie_links,src=id,trgt,weight,src_genre)#rename row and throw out old one


temp_df=select(movie_nodes,trgt=name,id,trgt_genre=genre)#temporary frame to make the id join easier
movie_links=inner_join(movie_links, temp_df, by = "trgt")#do the id join
movie_links$link_genre=rep("None",length(movie_links$src))
which_links=which(movie_links$src_genre==movie_links$trgt_genre)
movie_links$link_genre[which_links]=movie_links$src_genre[which_links]
movie_links=select(movie_links,src,trgt=id,weight,link_genre)#rename row and throw out old 
...
col_genre =rep("grey40", vcount(movie_network))#intializes color vector for vertex (nodes)
col_studio=rep("grey40", vcount(movie_network))#intializes color vector for vertex (nodes)
col_genre_link=rep("grey40", ecount(movie_network))#intializes color vector for edges(links)
...
#assign colors based on the type link (are both actors primarly in comedy, etc)
col_genre_link[E(movie_network)$link_genre=="Comedy"] = "orange"
col_genre_link[E(movie_network)$link_genre=="Drama"] = "blue"
col_genre_link[E(movie_network)$link_genre=="Horror"] = "black"
col_genre_link[E(movie_network)$link_genre=="Action"] = "green"
col_genre_link[E(movie_network)$link_genre=="Romance"] = "red"
col_genre_link[E(movie_network)$link_genre=="Fantasy"] = "purple"
...
plot(movie_network,vertex.label=NA,vertex.size=3,layout=l,
    vertex.color=col_genre,edge.color=col_genre_link)

forceNetwork(Links = movie_links, Nodes = movie_nodes, Source = "src", Target = "trgt",Value = "weight", NodeID = paste(",Group = "genre", opacity = 0.8)
