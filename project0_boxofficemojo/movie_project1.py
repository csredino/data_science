...
movies1=pandas.read_csv('movie_data.csv',usecols=[3,5,6])

movies1 = movies1[movies1['release'].str.contains(',')==True]#filter all bad dates and foreign only films
movies1['release']=map(lambda x:datetime.strptime(x, '%B %d, %Y'),movies1['release'])#get dates in correct format
movies1['week_number']=map(lambda x:datetime.strftime(x, '%W'),movies1['release'])#get the week number
